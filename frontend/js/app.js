...
const App = (() => {
    ...
    const ROLES={assistant:'你是AI视觉助手，请用中文简洁回答用户的问题。',teacher:'你是一位耐心的英语老师。用户用中文提问时，你用英文回答并附中文解释。',interviewer:'你是一位专业的面试官。针对用户的表现给出建设性反馈。',friend:'你是用户的好朋友，聊天风格随意幽默。'};
    ...
    // ── Media ─────────────────────────────────────────────────────
    async function requestMedia(){try{_mediaStream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:1280},height:{ideal:720},facingMode:'user'},audio:{sampleRate:16000,channelCount:1,echoCancellation:true,noiseSuppression:true}});UI.refs.cameraPreview.srcObject=_mediaStream;UI.hidePlaceholder();return true}catch(e){UI.addErrorMessage(e.name==='NotAllowedError'?'权限被拒绝':e.name==='NotFoundError'?'未检测到设备':'设备访问失败');return false}}
    ...
    // ── WS handler ────────────────────────────────────────────────
    function handleWsMessage(type,payload){switch(type){case'user_speech':UI.addUserSpeech('🗣 '+payload.text);break;case'status':UI.setStatus(payload.state);_muted=(payload.state==='speaking'||payload.state==='thinking');UI.showInterrupt(payload.state==='speaking'||payload.state==='thinking');if(payload.state==='listening')Cost.startAudio();else Cost.stopAudio();updateCost();break;case'text_delta':UI.appendAiText(payload.text||'');Cost.textOutChars+=(payload.text||'').length;if(UI.isSubtitleEnabled()){_subtitleText+=payload.text||'';UI.setSubtitle(_subtitleText)}break;case'text_done':UI.finishAiBubble();_subtitleText='';chime();updateCost();break;case'audio_delta':Cost.audioOutChars+=Math.round((payload.data||'').length*.25);if(UI.isAudioEnabled())_audioOut.addChunk(payload.data||'');break;case'audio_done':UI.showInterrupt(false);if(UI.isAudioEnabled()&&_audioOut.bufferLength>0)_audioOut.play();updateCost();break;case'error':UI.addErrorMessage(payload.message||'AI错误');UI.setStatus('error');UI.showInterrupt(false);break}}

    function interrupt(){if(_ws?.isConnected)_ws.send({type:'cancel'});UI.showInterrupt(false);UI.addSystemMessage('已打断')}

    // ── Start / Stop ──────────────────────────────────────────────
    async function start(){if(_state===State.RUNNING)return;_state=State.STARTING;UI.setStatus('connecting');Cost.reset();updateCost();if(!await requestMedia()){_state=State.ERROR;UI.setStatus('error');UI.setButtons(true,false);return}_ws=new WsClient();_ws.onMessage=handleWsMessage;_ws.connect();_audioOut=AudioPlayback;await AudioCapture.start(_mediaStream);AudioCapture.onChunk=b64=>{if(!_muted&&_ws?.isConnected)_ws.send({type:'audio',data:b64})};AudioCapture.onLevel=l=>{UI.setMeterLevel(l);drawWaveform(l)};VideoCapture.start(UI.refs.cameraPreview,UI.refs.captureCanvas);VideoCapture.onFrame=b64=>{if(_cameraOn&&_ws?.isConnected){_ws.send({type:'video',data:b64});Cost.videoFrames++}};UI.refs.quickAsks.style.display='flex';setTimeout(()=>{if(_ws?.isConnected)_ws.send({type:'start_session',instructions:getInstructions()})},600);_state=State.RUNNING;UI.setStatus('listening');UI.setButtons(false,true);UI.refs.btnCamera.disabled=false;UI.addSystemMessage('摄像头可拖拽/缩放。右下角 ↔ 调整大小。')}
